...
class QFT:
    ...
    def __init__(self, signal_length=16, 
        ...
        for k, q_s in enumerate(self.basis_to_transform):
            if int(q_s) == 1:
                # Change the qubit state form 0 to 1
    ...
    def qft_circuit_iter(self):
        ...
        if self.qubit_index > 0:
            ...
            for j in range(self.qubit_index):
                ...
                self.circuit.append(cirq.CZ(self.qubits[self.qubit_index], self.qubits[j]) ** rotation_to_apply)
    ...
    def qft_circuit(self):
        ...
        while self.qubit_index < self.num_qubits:
            ...
            print(self.circuit)
        # Swap the qubits to match qft definition 
        self.swap_qubits()
...
def main(signal_length = 16, basis_to_transform='0000', validate_inverse_fourier = False):

    # Instantiate QFT Class
    _qft_ = QFT(signal_length=signal_length, basis_to_transform=basis_to_transform, validate_inverse_fourier=validate_inverse_fourier)
